    ...
    static class Program
    {
        ...
        public static void CreateIfMissingAsync()
        {
            ...
            try
            {
                if (!File.Exists(@"C:\SMS\AppData\CKM.ini"))
                {
                    ...
                    text = text.Replace("=TennicMainMenuLogin", "=HaspoMainMenuLogin");
                    text = text.Replace("=HaspoMainMenuLogin", "=HaspoMainMenuLogin");
                    text = text.Replace("=HaspoStoreMenuLogin", "=HaspoMainMenuLogin");
                    text = text.Replace("=CapitalStoreMenuLogin", "=HaspoMainMenuLogin");
                    text = text.Replace("=CapitalMainMenuLogin", "=HaspoMainMenuLogin");
                    File.WriteAllText(@"C:\SMS\AppData\CKM.ini", text);
                }
                else
                {
                    string text = File.ReadAllText(@"C:\SMS\AppData\CKM.ini");
                    if (text.Contains("=HaspoMainMenuLogin") || text.Contains("=HaspoStoreMenuLogin"))
                    {
                        //ok
                    }
                    else
        ...
        static Form LoginFormName()
        {
            ...
            if (!System.Deployment.Application.ApplicationDeployment.IsNetworkDeployed)
            {
                ...
                localpath = localpath.Replace("HaspoSMS\\", "");
            }
            ...
            if (lbl.ReadConfig())
            {
                ...
                {
                    pgname = new HaspoStoreMenuLogin();
                ...
                {
                    MessageBox.Show("The program cannot initialize with the specified server instances  inside CKM.ini file. Please make sure Haspo's Store or MainMenu login type inside ini file!");
                    Environment.Exit(0);
                }
            }
