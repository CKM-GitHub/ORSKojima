...
namespace TanaoroshiNyuuryoku
{
    ...
    internal partial class TanaoroshiNyuuryoku : FrmMainForm
    {
        ...
        protected override void ExecDisp()
        {
            ...
            SetData(dt);
            if (dt.Rows.Count > 0)      //TaskNo3354 HET
            {
        ...
        private void DataToGrid()
        {
            try
            {
                ...
                if (!System.IO.Directory.Exists(AfterFileName))
                {
                    System.IO.Directory.CreateDirectory(AfterFileName);
                }
                ...
                //DataTable dtFilecopy = dtFile.Copy();
                ...
                foreach (string name in names)
                {
                    ...
                    if (!System.IO.File.Exists(AfterFileName + @"\" + System.IO.Path.GetFileName(name)))
                    {
                    ...
                    {
                        System.IO.File.Delete(AfterFileName + @"\" + System.IO.Path.GetFileName(name));
                        System.IO.File.Move(name, AfterFileName + @"\" + System.IO.Path.GetFileName(name));
                    }

                }
                //dtFile.Columns.Add("AQ");
                //if (dtFile.Rows.Count > dtFilecopy.Rows.Count)
                //{
                //    int a = dtFile.Rows.Count - dtFilecopy.Rows.Count;
                //    for (int i = 0; i < a; i++)
                //    {
                //        DataRow row = dtFilecopy.NewRow();
                //        row["AQ"] = 0;
                //        dtFilecopy.Rows.Add(row);
                //    }                   
                //}
               
                //for (int i = 0; i < dtFile.Rows.Count; i++)
                //{
                //   for(int j=0; j<dtFilecopy.Rows.Count; j++)
                //    {
                //        dtFile.Rows[i]["AQ"] = dtFilecopy.Rows[i]["AQ"];
                //    }
                //}

              
                //dtFilecopy.Merge(dtFile);
                    ...
                    while (!csvReader.EndOfData)
                    {
                        ...
                        else
                        {
                            ...
                            if (selectRow != null)
                            {
                                ...
                                //rw["col" + (int)EcsvCol.ActualQuantity] = selectRow["TheoreticalQuantity"].ToString();
                            }
                            csvData.Rows.Add(rw);
                        }
                    }   //While

                    if (bbl.ShowMessage("Q333") == DialogResult.Yes)
                    {
                        ...
                        {
                            foreach (DataRow data in dtFilecopy.Rows)
                            {
                                if (dr["RackNO"].ToString() == data["RackNO"].ToString() && dr["JANCD"].ToString() == data["JANCD"].ToString())
                                {
                                    dr["ActualQuantity"] = Convert.ToInt32(dr["ActualQuantity"].ToString()) + Convert.ToInt32(data["ActualQuantity"].ToString());
                                }
                            }
                        }
                    }
        ...
        private void Scr_Clr(short Kbn)
        {
            if (Kbn == 0)
            {
                ...
                BtnSubF11.Enabled = true;       //TaskNo3354 HET
