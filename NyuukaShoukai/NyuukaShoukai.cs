...
namespace NyuukaShoukai
{
    public partial class FrmNyuukaShoukai : FrmMainForm
    {
        ...
        private void F11()
        {
            if(ErrorCheck())
            {
                ...
                if(dtSearch.Rows.Count > 0)
                {
                    ...
                    dgvNyuukaShoukai.DataSource = ModifiedData(dt);
        ...
        private DataTable ModifiedData(DataTable dt)
        {            string ArrivalNO = "", Number = "", ArrivalSu = "",ArrivalPlanSu = "";
            foreach (DataRow row in dt.Rows)
            {
                //if (row["ArrivalDate"].ToString() == ArrivalDate && row["CalcuArrivalPlanDate"].ToString() == CalcuArrivalPlanDate && row["PurchaseDate"].ToString() == PurchaseDate && row["Goods"].ToString() == Goods && row["SKUCD"].ToString() == SKUCD && row["JanCD"].ToString() == JanCD && row["SKUName"].ToString() == SKUName)
                if(row["ArrivalNO"].ToString() == ArrivalNO && row["Number"].ToString() == Number)
                {
                    row["ArrivalDate"]="";
                    row["CalcuArrivalPlanDate"] = "";
                    row["PurchaseDate"] = "";
                    row["Goods"] = "";
                    row["SKUCD"] = "";
                    row["JanCD"] = "";
                    ...
                    row["ColorName"] = "";
                    row["SizeName"] = "";
                    if(row["ArrivalPlanSu"].ToString() == ArrivalPlanSu)
                    {
                        row["ArrivalPlanSu"] = "";
                    }
                ...
                else
                {
                    ...
                    //ArrivalDate = row["ArrivalDate"].ToString();
                    //CalcuArrivalPlanDate = row["CalcuArrivalPlanDate"].ToString();
                    //PurchaseDate = row["PurchaseDate"].ToString();
                    //Goods = row["Goods"].ToString();
                    //SKUCD = row["SKUCD"].ToString();
                    //JanCD = row["JanCD"].ToString();
                    //SKUName = row["SKUName"].ToString();
                    //Color = row["ColorName"].ToString();
                    //Size = row["SizeName"].ToString();
                    ArrivalPlanSu = row["ArrivalPlanSu"].ToString();
                    Number = row["Number"].ToString();
                }

                if (row["ArrivalNO"].ToString() == ArrivalNO  && row["ArrivalSu"].ToString() == ArrivalSu)
                {
                    row["ArrivalSu"] = "";
                }
                else
                {
                    ArrivalNO = row["ArrivalNO"].ToString();
                    ArrivalSu = row["ArrivalSu"].ToString();
                }
            }
            return dt;
        }
        ...
        private void statusChk1_CheckedChanged(object sender, EventArgs e)
        {
            if (!statusChk1.Checked)
            {
                txtPurchaseDate1.Enabled = false;
                txtPurchaseDate2.Enabled = false;
            }
            else
            {
                txtPurchaseDate1.Enabled = true;
                txtPurchaseDate2.Enabled = true;
            }
        }
    }
}
