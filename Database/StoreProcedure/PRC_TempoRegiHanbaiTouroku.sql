...
CREATE PROCEDURE [dbo].[PRC_TempoRegiHanbaiTouroku]
    ...
    @Operator  varchar(10),
    @PC  varchar(30),
    @ClearExpiredPoint varchar(30), --TaskNo3219 HET
...
BEGIN
    ...
    DECLARE @OldPurchaseAmount money;
    DECLARE @OldTaxAmount money;
    DECLARE @OldDiscountAmount money;
    DECLARE @OldBillingAmount money;
    DECLARE @OldPointAmount money;
    DECLARE @OldCardDenominationCD varchar(3);
    ...
    SET @PointRate=(SELECT top 1 PointRate 
    FROM M_StorePoint AS M 
    WHERE M.StoreCD = @StoreCD
    AND M.ChangeDate <= @ChangeDate
    ORDER BY M.ChangeDate desc);			--TaskNo3236 HET
            
    --ç°âÒÇÃîÃîÑÇ≈ì¡âøïiÇèúÇ¢ÇΩñ{ëÃäzÇ…ëŒÇµÇƒê›íËÇµÇΩó¶Ç≈É|ÉCÉìÉgÇïtó^Ç∑ÇÈ
    If @PointRate > 0			--TaskNo3236 HET
		SET @LastPoint = FLOOR(@SUMGaku * @PointRate/100);
    If @PointRate = 0			--TaskNo3236 HET
		SET @LastPoint = '0';
    ...
    BEGIN
     ...
     VALUES
        ...
        INSERT INTO [D_JuchuuDetails]
             ...
             SELECT @JuchuuNO                         
                   ...
                   ,tbl.SKUCD
                   ,tbl.JanCD
                   ,(SELECT top 1 CONVERT(varchar(80), M.SKUName) AS SKUName FROM M_SKU AS M
     ...
     VALUES
        ...
        IF @CashAmount <> 0
        BEGIN
        	----ÉeÅ[ÉuÉãì]ëóédólÇdáA(í èÌ) insertìXï‹ì¸èoã‡óöóÅ@D_DepositHistory  åªã‡Ç†ÇËÇÃéû    
        ...
        IF @ClearExpiredPoint ='yes'		--TaskNo3219 HET
		BEGIN
		 --ÉeÅ[ÉuÉãì]ëóédólÇk âÔàıÉ}ÉXÉ^Å[
        UPDATE M_Customer SET
            LastPoint =  @LastPoint - (CASE @Haspo WHEN 1 THEN @PointAmount ELSE 0 END) --É|ÉCÉìÉgè¡âª
		...
		IF @ClearExpiredPoint ='no'		--TaskNo3219 HET
		BEGIN
		 --ÉeÅ[ÉuÉãì]ëóédólÇk âÔàıÉ}ÉXÉ^Å[
        UPDATE M_Customer SET
    ...
    BEGIN
        ...
        UPDATE M_Customer SET
            LastPoint = LastPoint - @LastPoint + (CASE @Haspo WHEN 1 THEN @PointAmount ELSE 0 END) --É|ÉCÉìÉgè¡âªÅiéÊè¡Åj
            ,TotalPoint = TotalPoint - @LastPoint
            ,TotalPurchase = TotalPurchase - (@SalesGaku - @SalesTax)
            ,UpdateOperator = @Operator
            ,UpdateDateTime = @SYSDATETIME
       ...
       WHERE DM.JuchuuNO = @JuchuuNO
        ...
        INSERT INTO [D_DepositHistory]
     	...
     	SELECT
           ...
           ,tbl.SKUCD	--SKUCD, varchar(30),>
           ,tbl.JanCD	--JanCD, varchar(13),>
           ,-1 * tbl.JuchuuSuu  --SalesSU, money,>
           ,tbl.JuchuuUnitPrice --SalesUnitPrice, money,>
           ,-1 * tbl.JuchuuHontaiGaku   --SalesGaku, money,>
           ,-1 * tbl.JuchuuTax  --SalesTax, money,>
           ,tbl.JuchuuTaxRitsu  --SalesTaxRate, int,>
           ,-1 * tbl.JuchuuGaku --TotalGaku, money,>
           ,0	--Refund, money,>
           ,-1 * tbl.ProperGaku                                     --2019.12.12 add
           ,-1 * (tbl.ProperGaku - tbl.JuchuuGaku) AS DiscountGaku  --2019.12.12 add
           ,0	--IsIssued, tinyint,>
           ,@Program	--Program, varchar(100),>
           ,@Operator
           ,@SYSDATETIME
    ...
    BEGIN
        ...
        SELECT TOP 1 
             @OldPurchaseAmount = DS.PurchaseAmount
            ,@OldTaxAmount = DS.TaxAmount
            ,@OldDiscountAmount = DS.DiscountAmount
            ,@OldBillingAmount = DS.BillingAmount
            ,@OldPointAmount = DS.PointAmount
            ,@OldCardDenominationCD = DS.CardDenominationCD
            ,@OldCardAmount = DS.CardAmount
            ,@OldCashAmount = DS.CashAmount
            ,@OldDepositAmount = DS.DepositAmount
            ,@OldRefundAmount = DS.RefundAmount
            ,@OldCreditAmount = DS.CreditAmount
            ,@OldDenomination1CD = DS.Denomination1CD
            ,@OldDenomination1Amount = DS.Denomination1Amount
            ,@OldDenomination2CD = DS.Denomination2CD
            ,@OldDenomination2Amount = DS.Denomination2Amount
            ,@OldAdvanceAmount = DS.AdvanceAmount
            ,@OldTotalAmount = DS.TotalAmount
            ,@OldSalesRate = DS.SalesRate
        FROM D_StorePayment As DS
        WHERE SalesNO = @SalesNO
        AND StoreCD = @StoreCD
        ORDER BY SalesNORows DESC
        ;
        ...
        --ä˘ë∂ÇÃìXï‹ì¸èoã‡óöóÅ@D_DepositHistoryÅ@ÇÉåÉVÅ[Égî≠çsëŒè€äOÇ…Ç∑ÇÈ
        UPDATE [D_DepositHistory] SET NotReceiptFlg = 1 WHERE Number = @SalesNO

        --ÉeÅ[ÉuÉãì]ëóédólÇÉ(í˘ê≥)	Insert ìXï‹ì¸èoã‡óöóÅ@D_DepositHistory	îÑè„ÇÃÉwÉbÉ_èÓïÒ	  í˘ê≥ëO
        INSERT INTO [D_DepositHistory]
           ([StoreCD]
           ,[DepositDateTime]
           ,[DataKBN]
     	...
     	SELECT
           @StoreCD
           ,@SYSDATETIME  --DepositDateTime
           ,1   --DataKBN 1:îÃîÑèÓïÒÅA2:îÃîÑñæç◊èÓïÒÅA3:ì¸èoã‡èÓïÒ
           ,1   --DepositKBN 1;îÃîÑ,2:ì¸ã‡,3;éxï•,4:óºë÷ì¸,5:óºë÷èo,6:íﬁëKèÄîı,7:è§ïiåîíﬁ
       ...
       ;
		...
		INSERT INTO [D_JuchuuDetails]
             ...
             SELECT @JuchuuNO
                   ...
                   ,ISNULL(DM.ShippingPlanDate,@SYSDATETIME)
                   ,ISNULL(DM.SalesDate,@SYSDATETIME)
                   ,DM.SalesNO
                   ,DM.DepositeDetailNO
       ...
       ;
        ...
        INSERT INTO [D_StorePayment]
            ...
            SELECT
                   ...
                   ,@SalesTranIdent
                   ,@StoreCD
                   ,4	--1:í èÌÅA2:ï‘ïiÅA3;í˘ê≥ÅA4:éÊè¡
                   ,@OldPurchaseAmount
                   ,@OldTaxAmount
                   ,@OldDiscountAmount
                   ,@OldBillingAmount
