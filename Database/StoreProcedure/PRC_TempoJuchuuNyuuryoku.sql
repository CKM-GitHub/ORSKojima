...
BEGIN
    ...
    BEGIN
            ...
            -- ========= ループ内の実際の処理 ここから===
                ...
                UPDATE D_JuchuuDetails SET
                    ...
                    ,HikiateFlg = (CASE WHEN D_JuchuuDetails.HikiateSu - A.ReserveSu >= D_JuchuuDetails.JuchuuSuu THEN 1
                                ...
                                ELSE 0 END )
                FROM (SELECT R.[Number], R.NumberRows, SUM(R.ReserveSu) AS ReserveSu 
                    FROM D_Reserve AS R
                    WHERE R.ReserveKBN = 1
                    AND R.DeleteDateTime IS NULL
                    AND R.[Number] = @updJuchuuNo
                	GROUP BY R.[Number], R.NumberRows)AS A  
        ...
        DECLARE @tempDenNO varchar(11)
        IF @OperateMode = 1
        BEGIN
            --新規時は受注番号(もしくは受注処理番号)がまだ採番されていないので、仮番号を採番する
            DECLARE @t AS TABLE (DenNO varchar(11))
            INSERT INTO @t EXEC GetTemporaryReserveNO ''
            SET @tempDenNO = (SELECT DenNO FROM @t)
        END
        ELSE IF @Tennic = 1 AND @OperateMode <> 1
        BEGIN
            --テニックの場合、明細ごとの受注番号を引当処理に引き渡す
            INSERT INTO @NumberTable SELECT JuchuuNO, JuchuuRows FROM @Table
        END
        --s.Akao add end
        ...
        DECLARE CUR_AAA CURSOR FOR
            SELECT tbl.SKUNO,tbl.SoukoCD,tbl.JuchuuSuu
                ...
                ,ISNULL(CASE @Tennic WHEN 1 THEN tbl.JuchuuNo ELSE @JuchuuNo END, @tempDenNO) AS JuchuuNo
                --S.Akao change end
            FROM @Table tbl
        ...
        -- ========= ループ内の実際の処理 ここから===
            ...
            BEGIN
                ...
                SELECT  
                    ...
                    ,tbl.CommentInStore
                    ,tbl.CommentOutStore
                    --直送 もしくは 引当済み(かつ入荷済み)なら1
                    ,CASE WHEN DM.DirectFLG = 1 OR ISNULL(DM.JuchuuSuu,0) <= ISNULL(Reserve.ReserveSu,0) THEN 1 ELSE 0 END
                    ,0   --UpdateCancelKBN]
                ...
                FROM @Table tbl
                INNER JOIN D_JuchuuDetails AS DM
                ON  DM.JuchuuNO = @tblJuchuuNO
                AND DM.JuchuuRows = 1
                LEFT OUTER JOIN 
                (
                    SELECT R.Number, R.NumberRows, SUM(R.ReserveSu) AS ReserveSu
                    FROM D_Reserve R
                    INNER JOIN D_Stock S
                    ON  S.StockNO = R.StockNO
                    AND S.ArrivalYetFLG = 0 --入荷済み
                    AND S.DeleteDateTime IS NULL
                    WHERE R.Number = @tblJuchuuNO
                    AND R.ReserveKBN = 1
                    AND R.DeleteDateTime IS NULL
                    GROUP BY R.Number, R.NumberRows
                ) Reserve
                ON Reserve.Number = DM.JuchuuNO
                AND Reserve.NumberRows = DM.JuchuuRows
                WHERE tbl.DisplayRows = @DisplayRows
                AND tbl.UpdateFlg = 0
                ;                 
                ...
                UPDATE [D_DeliveryPlanDetails] SET
                        [CommentInStore]  = tbl.CommentInStore
                        ,[CommentOutStore] = tbl.CommentOutStore
                        ,[HikiateFLG]      = CASE WHEN DM.DirectFLG = 1 OR ISNULL(DM.JuchuuSuu,0) <= ISNULL(Reserve.ReserveSu,0) 
                                                THEN 1 ELSE 0 END
                        ,[UpdateOperator]  =  @Operator  
                        ,[UpdateDateTime]  =  @SYSDATETIME
                ...
                ON Reserve.Number = DM.JuchuuNO
                AND Reserve.NumberRows = DM.JuchuuRows
                WHERE D_DeliveryPlanDetails.Number = tbl.JuchuuNO
                AND D_DeliveryPlanDetails.Number = @tblJuchuuNO
                AND D_DeliveryPlanDetails.NumberRows = 1
                AND tbl.JuchuuNO = @tblJuchuuNO
