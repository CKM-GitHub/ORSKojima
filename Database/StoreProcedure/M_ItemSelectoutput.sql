...
Create PROCEDURE [dbo].[M_ItemSelectoutput]
	...
	@TagName1 as varchar(40),
	@TagName2 as varchar(40),
	@TagName3 as varchar(40),
	@TagName4 as varchar(40),
	@TagName5 as varchar(40),
	@type as tinyint,
	@chktype as tinyint,
...
BEGIN
	...
	SET @SQL = '
		...
		, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] =''226''  and [Key]= fim._ExhibitionSegmentCD) as ''セグメント名''
	...
	, (select top 1 LEFT (Char1, 20)  from M_MultiPorpose mp where mp.[ID] =''316'' and [Key]=fim.OrderAttentionCD) as ''発注注意区分名''
	...
	,fim.TagName10 as ''ITEMタグ10''
		...
		EXECUTE sp_executesql @SQL, N'@ChangeDate DATE,@VendorCD VARCHAR(13),@makerCD VARCHAR(13),@brandCD VARCHAR(6),@SKUName VARCHAR(100),@JanCD VARCHAR(13),@SkuCD VARCHAR(40),@MakerItem VARCHAR(300),@ItemCD VARCHAR(300),@CommentInStore VARCHAR(300),@ReserveCD VARCHAR(3),@NticesCD VARCHAR(3),@PostageCD VARCHAR(3),@OrderAttentionCD VARCHAR(3),@SportsCD VARCHAR(6),@InsertDateTimeT DATETIME,@InsertDateTimeF DATETIME,@UpdateDateTimeT DATETIME,@UpdateDateTimeF DATETIME,@ApprovalDateF DATETIME,@ApprovalDateT DATE,@YearTerm as varchar(6),@Season VARCHAR(6),@CatalogNo VARCHAR(50),@InstructionsNO VARCHAR(1000),@TagName1 VARCHAR(40),@TagName2 VARCHAR(40),@TagName3 VARCHAR(40),@TagName4 VARCHAR(40),@TagName5 VARCHAR(40),@chkUnapprove TINYINT',
		...
		@ReserveCD = @ReserveCD, @NticesCD = @NticesCD, @PostageCD = @PostageCD, @OrderAttentionCD = @OrderAttentionCD, @SportsCD = @SportsCD, @InsertDateTimeF = @InsertDateTimeF, @InsertDateTimeT = @InsertDateTimeT, @UpdateDateTimeF = @UpdateDateTimeF, @UpdateDateTimeT = @UpdateDateTimeT, @ApprovalDateF = @ApprovalDateF, 
		@ApprovalDateT = @ApprovalDateT, @YearTerm = @YearTerm, @Season = @Season, @CatalogNo = @CatalogNo, @InstructionsNO = @InstructionsNO, @TagName1 = @TagName1, @TagName2 = @TagName2, @TagName3 = @TagName3, @TagName4 = @TagName4, @TagName5 = @TagName5, @chkUnapprove = @chkUnapprove 
