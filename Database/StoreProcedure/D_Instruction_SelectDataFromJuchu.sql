    ...
    SELECT  DJ.ExpressFLG
        ...
        FROM (SELECT DM.ExpressFLG
           ...
           --AND DD.DeleteDateTime IS NULL
            ...
            WHERE ((@DeliveryPlanDateFrom is null or DM.ShippingPlanDate >=CONVERT(DATE, @DeliveryPlanDateFrom))
			AND (@DeliveryPlanDateTo is null or DM.ShippingPlanDate <=CONVERT(DATE, @DeliveryPlanDateTo)) or DM.ShippingPlanDate is null) 
			--DM.ShippingPlanDate >= (CASE WHEN @DeliveryPlanDateFrom <> '' THEN CONVERT(DATE, @DeliveryPlanDateFrom) ELSE DM.ShippingPlanDate END)
