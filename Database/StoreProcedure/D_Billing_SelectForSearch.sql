...
BEGIN
    ...
    BEGIN
        SELECT MAX(W.StoreCD) AS StoreCD
              ...
              ,SUM(W.BillingGaku) AS BillingGaku
			   ,CASE
			   WHEN MAX(W.BillingType) =2  AND MAX(W.BillingConfirmFlg)=0  THEN '未確認'
				ELSE ''
			END AS BillingFlg
              ,MAX(W.CollectDate) AS CollectDate
			  ,SUM(W.CollectAmount) As CollectAmount
			  ,SUM(W.UnpaidAmount) AS UnpaidAmount
        FROM (
            SELECT DB.StoreCD
                  ...
                  ,DB.BillingNO
                  ,ISNULL(DBM.BillingGaku,0) AS BillingGaku
				  ,DB.BillingType AS BillingType
				  ,DB.BillingConfirmFlg AS BillingConfirmFlg
                  ,CONVERT(varchar,DB.CollectDate,111) AS CollectDate
				  ,ISNULL(DC.CollectAmount,0) AS CollectAmount
				  ,DBM.BillingGaku - ISNULL(DC.CollectAmount,0) AS UnpaidAmount
    ...
    BEGIN
        ...
        FROM (
            SELECT DB.StoreCD
                 ,(SELECT top 1 A.StoreName 
                  ...
                  ,(SELECT top 1 A.CustomerName
				   ...
				   ,ISNULL(DSM.SalesGaku,0) -ISNULL(DCBD.CollectAmount,0) AS UnpaidAmount
			...
			ON DCBD.CollectPlanNO=DBM.CollectPlanNO
			AND DCBD.CollectPlanRows=DBM.CollectPlanRows
