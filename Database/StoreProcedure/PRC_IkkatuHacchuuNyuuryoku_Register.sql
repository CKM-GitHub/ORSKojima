...
*****************************************/    
   ...
   ,DeleteDateTime          
    ...
    SELECT OrderNO                  = CASE WHEN @p_OperateMode = 1 THEN CAST(DENSE_RANK()OVER(ORDER BY MAIN.SiiresakiCD,MAIN.NounyuusakiName,MAIN.NounyuusakiJuusho) AS varchar) ELSE MAIN.HacchuuNO END --新規の場合は後で採番、キーだけふっとく    
          ...
          ,Remarks                  = CAST(MAIN.HacchuuChuuiZikou as varchar(30))      --HET TaskNo3156                      
...
IF @p_OperateMode = 1      
 ...
 --その（受注番号＋明細番号の）商品の手配ができないため最初に発注した時と違う仕入先に発注した場合に、以前の発注先に対して「キャンセル」の発注を自動的に行うための処理  
        ...
        --HET from myanmar (TaskNo2890)
		--D_JuchuuDetails
		UPDATE D_JuchuuDetails 
		SET		JuchuuOrderNO		= DOD.OrderNO,
				VendorCD			= DO.OrderCD,
				IrregularOrderFLG	= 0,
				LastOrderNO			= DOD.OrderNO,
				LastOrderRows		= DOD.OrderRows,
				LastOrderDateTime   = @SYSDATETIME,
				UpdateOperator		= @p_Operator,
				UpdateDateTime		= @SYSDATETIME
		FROM D_JuchuuDetails DJD
		INNER JOIN D_OrderDetails DOD
		ON DJD.JuchuuNO = DOD.JuchuuNO 
		AND DJD.JuchuuRows = DOD.JuchuuRows
		AND DJD.DeleteDateTime IS NULL 
		INNER JOIN D_Order DO
		ON DO.OrderNO = DOD.OrderNO 
