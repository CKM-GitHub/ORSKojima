...
namespace MasterTouroku_SKUCDHenkou_SKUCD変更
{
    public partial class MasterTouroku_SKUCDHenkou_SKUCD変更 : FrmMainForm
    {
        ...
        {
            InProgramID = "MasterTouroku_SKUCDHenkou";
            ...
            //dgvHenkou.MouseWheel += new MouseEventHandler(dgvHenkou_MouseWheel);
        }

        class newLabel : System.Windows.Forms.Label
        {
            public int RotateAngle { get; set; }  // to rotate your text
            public string NewText { get; set; }   // to draw textd
        ...
        private void SetDesignerFunc()
        {
            ...
            dgvHenkou.Rows[0].Selected = false;
        }
        ...
        private void Panel2_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            if (e.Delta > 0 && dgvHenkou.FirstDisplayedScrollingColumnIndex > 0)
            {
                dgvHenkou.FirstDisplayedScrollingColumnIndex--;
            }
            else if (e.Delta < 0)
            {
                dgvHenkou.FirstDisplayedScrollingColumnIndex++;
        ...
        {
            if (e.Delta > 0 && dgvHenkou.FirstDisplayedScrollingRowIndex > 0)
            {
                dgvHenkou.FirstDisplayedScrollingRowIndex--;
            }
            else if (e.Delta < 0)
            {
                dgvHenkou.FirstDisplayedScrollingRowIndex++;
        ...
        private void Ct_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {              
                ...
                if (ActiveControl is CKM_TextBox ct && (ct.Name.Contains("sn_") | ct.Name.Contains("cn_")))
                {
                    ...
                    {
                        TextBox tbx = this.Controls.Find("sn_" + (Convert.ToInt32(ct.Name.Split('_').Last()) + 1), true).FirstOrDefault() as TextBox;
                        ct.TabIndex = Convert.ToInt32(ct.Name.Split('_').Last());
                        if (tbx.Enabled == true)
                        {
                            if (ct.TabIndex != iColumn)
                            {
                                for (int i = iColumn; i < ct.TabIndex; i++)
                                {
                                    dgvHenkou.Rows[++iColumn].ToString();
                                }
                                dgvHenkou.CurrentCell = dgvHenkou[iColumn, iRow];
                                dgvHenkou.CurrentCell.Selected = false;
                            }
                        }
                        else
                        {                          
                    ...
                    {         
                        TextBox tbx = this.Controls.Find("cn_" + (Convert.ToInt32(ct.Name.Split('_').Last()) + 1), true).FirstOrDefault() as TextBox;
                        ct.TabIndex = Convert.ToInt32(ct.Name.Split('_').Last());
                        if(tbx.Enabled == true)
                        {
                            if (ct.TabIndex != iRow)
                            {
                                for (int i = iRow; i < ct.TabIndex; i++)
                                {
                                    dgvHenkou.Rows[++iRow].ToString();
                                }
                                dgvHenkou.CurrentCell = dgvHenkou[iColumn, iRow];
                                dgvHenkou.CurrentCell.Selected = false;
                            }
                ...
                //if(!IsStop)Check_E280(true);    // Check Biggest
            }
        }
        static bool IsStop = false;
        private void Check_E280(bool IsEnterTrigger = false)
        {
            int count = 0;
            string Con = "";
            var id = "";
        ...
        {
            for (int i = 0; i < dgvHenkou.Columns.Count; i++)
            {
                for (int j = 0; j < dgvHenkou.Rows.Count; j++)
                {
                    dgvHenkou.Rows[j].Cells[i].Value = "";
                    dgvHenkou.Rows[j].Cells[i].Selected = false;
                }
            }
        ...
        private void F11()
        {
            ...
            if (ErrorCheck(11))
            {
                ...
                dgvHenkou.Refresh();
                dgvHenkou.FirstDisplayedScrollingRowIndex = 0;
                dgvHenkou.FirstDisplayedScrollingColumnIndex = 0;
                dgvHenkou.ReadOnly = true;
        ...
        private void Set_JanCD(DataTable dt)
        {
            ...
            dgvHenkou.ShowCellToolTips = false;
            for (int l = 0; l < dt.Rows.Count; l++)
            {
                var c = Convert.ToInt32(dt.Rows[l]["SizeNo"].ToString());
                var r = Convert.ToInt32(dt.Rows[l]["ColorNo"].ToString());
                dgvHenkou[c - 1, r - 1].Value = dt.Rows[l]["JanCD"].ToString();
                if(dt.Rows[l]["VirtualFlag"].ToString().Equals("1"))
                {
                    dgvHenkou[c - 1, r - 1].Style.BackColor = Color.HotPink; 
                }
                else
                {
                    if (dgvHenkou[c - 1, r - 1].RowIndex % 2 != 0)
                        dgvHenkou[c - 1, r - 1].Style.BackColor = Color.FromArgb(221, 235, 247);
                    else dgvHenkou[c - 1, r - 1].Style.BackColor = SystemColors.Window;
                }
            }
            dgvHenkou.Rows[0].Selected = false;
        }
        private void Set_Value(DataTable dt, Panel pnl)
        {
            ...
            var c = GetAllControls(pnl);   /// Size Count Position
                ...
                else
                {
                        ...
                        var dt_Color = dt.AsEnumerable().GroupBy(r => r.Field<string>("ColorName")).Select(g => g.First()).CopyToDataTable();
        ...
        private void Sc_Item_CodeKeyDownEvent(object sender, KeyEventArgs e)
        {
            if(e.KeyCode == Keys.Enter)
            {
                ...
                if (OperationMode == EOperationMode.INSERT)
                {
                    txtRevDate.Text = Sc_Item.ChangeDate;
                }
                else if (OperationMode == EOperationMode.UPDATE)
                {
                    txtDate1.Text = Sc_Item.ChangeDate;
                }
        ...
        //        return;
        //    }
        //    panel4.VerticalScroll.Value = (dgvHenkou.FirstDisplayedScrollingRowIndex * 18 +1);
        //    panel2.HorizontalScroll.Value= (dgvHenkou.FirstDisplayedScrollingColumnIndex * 130 + 1);
        //}
        ...
        private void EnterVoid()
        {
            ...
            foreach (var con in c)
            {
                ...
                {
                    ct.KeyDown += Ct_Enter;
                    ct.TextAlign = HorizontalAlignment.Center;
                }
            }
        ...
        private void EnterVoid1()
        {
            var s = GetAllControls(panel2);

            foreach (var con in s)
            {

                if (con is CKM_TextBox ct && (ct.Name.Contains("so_")))
                {
                    ct.TextAlign = HorizontalAlignment.Center;
                }
            }
            ...
            foreach (var a in c)
            {
                if (a is CKM_TextBox ct && (ct.Name.Contains("co_")))
                {
                    ct.TextAlign = HorizontalAlignment.Center;
                }
            }
        ...
        private void label10_Paint_1(object sender, PaintEventArgs e)
        {
            Font font = new Font(
            label10.Font.FontFamily,
            9,
            FontStyle.Regular,
            GraphicsUnit.Point);
            PointF pointF = new PointF(15, 150);
            StringFormat stringFormat = new StringFormat();
            SolidBrush solidBrush = new SolidBrush(Color.Black);
            stringFormat.FormatFlags = StringFormatFlags.DirectionVertical;
            e.Graphics.DrawString("カ ラ ー", font, solidBrush, pointF, stringFormat);
        }

        private void dgvHenkou_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            dgvHenkou.Rows[0].Selected = false;
        }

        private void dgvHenkou_Scroll(object sender, ScrollEventArgs e)
        {
            if (e.NewValue == 31)
            {
                return;
