...
namespace MasterTorikomi_SKU
{
    public partial class MasterTorikomi_SKU : FrmMainForm
    {
        ...
        private bool Is282(string val, int len)
        {

            if (val == null)
                val = "";
            var japanLength = Encoding.GetEncoding(932).GetByteCount(val).ToString();
            return Convert.ToInt32(japanLength) > len;
        }
        ...
        private void F12()
        {
            try
            {
                if (bbl.ShowMessage("Q001", "取込処理") == DialogResult.Yes)
                {
                    ...
                    var dt = GV_SKU.DataSource as DataTable;
                    if (dt == null)
                    {
        ...
        private bool ErrorCheck(DataTable dt)
        {
            ...
            string kubun = dt.Rows[0]["データ区分"].ToString();
            if (RB_all.Checked)
            {
        ...
        private void ExcelErrorCheck(DataTable dt)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                {
                    var adminNO = dt.Rows[i]["AdminNo"].ToString();
                    if (adminNO.ToUpper() == "NEW")
                    {
                        dt.Rows[i]["AdminNo"] = "New";
                    }
                    else
                    {
                        if (!Int32.TryParse(adminNO, out var n))
                        {
                            dt.Rows[i]["EItem"] = "AdminNo";
                ...
                {
                    if (Is282(dt.Rows[i]["SKUCD"].ToString(), 40))
                    {
                        dt.Rows[i]["EItem"] = "SKUCD";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                    }
                }
                if (dt.Rows[i]["AdminNo"].ToString() != "New")
                {
                    var mske = new M_SKU_Entity()
                    {
                        AdminNO = dt.Rows[i]["AdminNo"].ToString(),
                        SKUCD = dt.Rows[i]["SKUCD"].ToString(),
                        ChangeDate = dt.Rows[i]["改定日"].ToString()
                    };
                    var result = mtbl.M_SKU_CheckSKUExists(mske);
                    if (!result)
                    {
                        dt.Rows[i]["EItem"] = "AdminNO";
                ...
                {
                    if (dt.Rows[i]["JANCD"].ToString().ToUpper() == "仮JAN")
                    {
                        dt.Rows[i]["JANCD"] = "仮JAN";
                    }
                    else if (dt.Rows[i]["JANCD"].ToString().ToUpper() == "自社JAN")
                    {
                        dt.Rows[i]["JANCD"] = "自社JAN";
                    }
                    else if (dt.Rows[i]["JANCD"].ToString().ToUpper() == "削除JAN")
                    {
                        dt.Rows[i]["JANCD"] = "削除JAN";
                    }
                    else
                    {
                        var jancd = dt.Rows[i]["JANCD"].ToString();
                        if (!Int64.TryParse(jancd, out var n))
                        {
                            dt.Rows[i]["EItem"] = "JANCD";
                            dt.Rows[i]["Error"] = "入力された値が正しくありません。";
                            continue;
                        }
                        else if (Is282(jancd, 13))
                        {
                            dt.Rows[i]["EItem"] = "JANCD";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                ...
                {
                    if (Is282(dt.Rows[i]["商品名"].ToString(), 80))
                    {
                        dt.Rows[i]["EItem"] = "商品名";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                ...
                {
                    if (Is282(dt.Rows[i]["ITEMCD"].ToString(), 30))
                    {
                        dt.Rows[i]["EItem"] = "ITEMCD";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                    ...
                    var mske = new M_SKU_Entity()
                    {
                        ITemCD = dt.Rows[i]["ITEMCD"].ToString(),
                        ChangeDate = dt.Rows[i]["改定日"].ToString()
                    };
                    var result = mtbl.M_SKU_CheckItemExists(mske);
                    if (!result)
                    {
                        dt.Rows[i]["EItem"] = "ITEMCD";
                        dt.Rows[i]["Error"] = "登録されていないコードです。";
                ...
                {
                    if (Is282(dt.Rows[i]["サイズ枝番"].ToString(), 2))
                    {
                        dt.Rows[i]["EItem"] = "サイズ枝番";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                ...
                {
                    if (Is282(dt.Rows[i]["カラー枝番"].ToString(), 2))
                    {
                        dt.Rows[i]["EItem"] = "カラー枝番";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                ...
                {
                    if (Is282(dt.Rows[i]["サイズ名"].ToString(), 32))
                    {
                        dt.Rows[i]["EItem"] = "サイズ名";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                ...
                if (String.IsNullOrEmpty(dt.Rows[i]["カラー名"].ToString()))
                {
                    dt.Rows[i]["EItem"] = "カラー名";
                    dt.Rows[i]["Error"] = "該当の値を入力・選択してください。";
                    continue;
                }
                else
                {
                    if (Is282(dt.Rows[i]["カラー名"].ToString(), 32))
                    {
                        dt.Rows[i]["EItem"] = "カラー名";
                        dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                        continue;
                ...
                if (RB_all.Checked)
                {
                    ...
                    {
                        if (Is282(dt.Rows[i]["カナ名"].ToString(), 50))
                        {
                            dt.Rows[i]["EItem"] = "カナ名";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["略名"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "略名";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["英語名"].ToString(), 80))
                        {
                            dt.Rows[i]["EItem"] = "英語名";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["メーカー商品CD"].ToString(), 30))
                        {
                    ...
                    {
                        if (Is282(dt.Rows[i]["発注注意事項"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "発注注意事項";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    if (!String.IsNullOrEmpty(dt.Rows[i]["管理用備考"].ToString()))
                    {
                        if (Is282(dt.Rows[i]["管理用備考"].ToString(), 80))
                        {
                            dt.Rows[i]["EItem"] = "管理用備考";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["表示用備考"].ToString(), 80))
                        {
                            dt.Rows[i]["EItem"] = "表示用備考";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["カタログ番号"].ToString(), 50))
                        {
                            dt.Rows[i]["EItem"] = "カタログ番号";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["カタログページ"].ToString(), 50))
                        {
                            dt.Rows[i]["EItem"] = "カタログページ";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["カタログ番号Long"].ToString(), 2000))
                        {
                            dt.Rows[i]["EItem"] = "カタログ番号Long";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["カタログページLong"].ToString(), 2000))
                        {
                            dt.Rows[i]["EItem"] = "カタログページLong";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["カタログ情報"].ToString(), 1000))
                        {
                            dt.Rows[i]["EItem"] = "カタログ情報";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["指示書番号"].ToString(), 500))
                        {
                            dt.Rows[i]["EItem"] = "指示書番号";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["商品情報アドレス"].ToString(), 200))
                        {
                            dt.Rows[i]["EItem"] = "商品情報アドレス";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ1"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ1";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ2"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ2";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ3"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ3";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ4"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ4";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ5"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ5";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。82";
                            continue;
                        }
                    }
                    if (!String.IsNullOrEmpty(dt.Rows[i]["タグ6"].ToString()))
                    {
                        if (Is282(dt.Rows[i]["タグ6"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ6";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ7"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ7";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ8"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ8";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ9"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ9";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                    ...
                    {
                        if (Is282(dt.Rows[i]["タグ10"].ToString(), 40))
                        {
                            dt.Rows[i]["EItem"] = "タグ10";
                            dt.Rows[i]["Error"] = "項目の長さが最大長を超えています。";
                            continue;
                        }
                    }
                ...
                if (RB_all.Checked || RB_BaseInfo.Checked || RB_attributeinfo.Checked || RB_JanCD.Checked)
                {
                    //ITemCD + サイズ枝番 + カラー枝番
                    var row = dt.Rows[i];
                    var existsExcel = dt.AsEnumerable().Where(e =>
                                        e["ITEMCD"].ToStringOrEmpty() == row["ITEMCD"].ToString()
                                        && e["サイズ枝番"].ToInt32(0) == row["サイズ枝番"].ToInt32(0)
                                        && e["カラー枝番"].ToInt32(0) == row["カラー枝番"].ToInt32(0)).Count() > 1;

                    var mske = new M_SKU_Entity()
                    {
                        AdminNO = row["AdminNo"].ToString() == "New" ? "0" : row["AdminNo"].ToString(),
                        ITemCD = row["ITEMCD"].ToString(),
                        SizeNO = row["サイズ枝番"].ToString(),
                        ColorNO = row["カラー枝番"].ToString(),
                        ChangeDate = row["改定日"].ToString()
                    };
                    var existsDB = mtbl.M_SKU_CheckDuplicateSizeAndColor(mske);

                    if (existsExcel || existsDB)
                    {
                        dt.Rows[i]["EItem"] = "ITEMCD,サイズ枝番,カラー枝番";
                        dt.Rows[i]["Error"] = GetMessage("E289");
                        continue;
                    }
                }
        ...
        {
            GV_SKU.DataSource = null;
            if (!System.IO.Directory.Exists("C:\\SMS\\MasterShutsuryoku_SKU\\"))
            {
                System.IO.Directory.CreateDirectory("C:\\SMS\\MasterShutsuryoku_SKU\\");
            }
            OpenFileDialog file = new OpenFileDialog(); 
            file.InitialDirectory = "C:\\SMS\\MasterShutsuryoku_SKU\\"; 
        ...
        {
            DataRow dr = dtMessage.AsEnumerable().Where(r => r.Field<string>("MessageID") == id).SingleOrDefault();
            return dr["MessageText1"].ToStringOrEmpty();
        }
    }
}
