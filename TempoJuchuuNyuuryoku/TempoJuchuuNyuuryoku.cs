...
namespace TempoJuchuuNyuuryoku
{
    ...
    internal partial class TempoJuchuuNyuuryoku : FrmMainForm
    {
        ...
        private bool InCaseOfLeaveEvent = false; //#Leave event
        private bool SuppressLeaveEvent = false; //#Leave event
        ...
        private void S_SetInit_Grid()
        {
            ...
            mGrid.PreInput = new ClsGridJuchuu.ST_DArray_Grid[mGrid.g_MK_Max_Row];
        ...
        private void S_Clear_Grid()
        {
            ...
            Array.Clear(mGrid.PreInput, mGrid.PreInput.GetLowerBound(0), mGrid.PreInput.GetLength(0));
                ...
                else
                {
                    ...
                    for (w_Col = mGrid.g_MK_State.GetLowerBound(0); w_Col <= mGrid.g_MK_State.GetUpperBound(0); w_Col++)
                    {
                        switch (w_Col)
                        {
                            ...
                            case (int)ClsGridJuchuu.ColNO.SoukoName:          //出荷倉庫
                                {
                                    //By default, the The shipping warehouse of the statement shall be read-only.
                                    mGrid.g_MK_State[w_Col, pRow].Cell_Enabled = false;
                                    mGrid.g_MK_State[w_Col, pRow].Cell_Bold = true;

                                    //Do not delete the source code for customization.
        ...
        private void DEL_SUB()
        {
            ...
            SuppressLeaveEvent = true;

            if (w_Act.GetType().Equals(typeof(CKM_Controls.CKM_Button)))
                w_Act = previousCtrl;

            if (mGrid.F_Search_Ctrl_MK(w_Act, out int w_Col, out int w_CtlRow) == false)
            {
                SuppressLeaveEvent = false;
                return;
            }
            ...
            for (int i = w_Row; i < mGrid.g_MK_Max_Row - 1; i++)
            {
                ...
                mGrid.PreInput[i] = mGrid.PreInput[i + 1];
        ...
        private void ADD_SUB()
        {
            ...
            for (int i = mGrid.g_MK_Max_Row - 1; i > w_Row; i--)
            {
                ...
                for (w_Col = mGrid.g_MK_State.GetLowerBound(0); w_Col <= mGrid.g_MK_State.GetUpperBound(0); w_Col++)
                {
                    mGrid.g_MK_State[w_Col, i] = mGrid.g_MK_State[w_Col, i - 1];
                }
            }
            ...
            Array.Clear(mGrid.PreInput, w_Row, 1);
        ...
        private void Grid_Gyo_Clr(int RW)  // 明細部１行クリア
        {
            ...
            Array.Clear(mGrid.PreInput, RW, 1);
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lblDisp.Visible = false; //Sales status is not displayed.
                Btn_F11.Text = "引当(F11)";
        ...
        private void InitialControlArray()
        {
            ...
            //受注日を変更したら即反映させたいため            
            detailControls[(int)EIndex.JuchuuDate].Leave += new System.EventHandler(DetailControl_Leave);
            ...
            else
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        //【Data Area Footer】
                        CboPaymentMethodCD.SelectedValue = row["PaymentMethodCD"].ToString();
                    ...
                    CheckGrid((int)ClsGridJuchuu.ColNO.JanCD, i, true, false, false, index);

                    if (mGrid.g_DArray[i].VariousFLG == 1)
                    {
                    ...
                    if (index != (int)EIndex.CopyJuchuuNO)
                    {
                        ...
                        if (!mGrid.g_DArray[i].ChkTyokuso)
                        {
                            var hikiate = bbl.Z_Set(row["HikiateFLG"]);
                            if (hikiate == 1)
                            {
                                mGrid.g_DArray[i].Hikiate = "引当OK";
                            }
                            else if (hikiate == 2)
                            {
                                mGrid.g_DArray[i].Hikiate = "一部OK";
                            }
                            else
                            {
                                if (mGrid.g_DArray[i].ZaikoKBN == 0)
                                {
                                    mGrid.g_DArray[i].Hikiate = "引当OK";
                                }
                                else
                                {
                                    mGrid.g_DArray[i].Hikiate = "引当NG";
                                }
                            }
                        }
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(mGrid.g_DArray[i].VendorCD))
                        CheckGrid((int)ClsGridJuchuu.ColNO.VendorCD, i, true, false, false, index);
                    else
                    ...
                    mGrid.CommitPreviousInput(i); //前回入力値を確定
        ...
        private bool CheckDataMitsumori(bool set)
        {
            ...
            {
                MitsumoriNO = keyControls[(int)EIndex.MitsumoriNO].Text
            };
            ...
            else
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        //【Data Area Footer】
                        //CboPaymentMethodCDはCheckDetail((int)EIndex.CustomerCD)でセットされる
                        detailControls[(int)EIndex.NouhinsyoComment].Text = row["RemarksOutStore"].ToString();

                        //明細なしの場合
                        if (bbl.Z_Set(row["MitsumoriRows"]) == 0) break;
                    }

                    if (string.IsNullOrWhiteSpace(row["JanCD"].ToString())) continue;

                    if (i == 0)
                    {
                        //1行目だけ「最新単価を取得し金額を再計算しますか？」
                        isReCalc = (bbl.ShowMessage("Q303") == DialogResult.Yes);
                    }

                    mGrid.g_DArray[i].JanCD = row["JanCD"].ToString();
                    mGrid.g_DArray[i].SKUCD = row["SKUCD"].ToString();
                    mGrid.g_DArray[i].AdminNO = row["SKUNO"].ToString();
                    mGrid.g_DArray[i].JuchuuSuu = bbl.Z_SetStr(row["MitsumoriSuu"]); //単価取得再計算のため先にセット
                    CheckGrid((int)ClsGridJuchuu.ColNO.JanCD, i, true, false, false, (int)EIndex.MitsumoriNO);
                    ...
                    M_SKULastCost_Entity msce = new M_SKULastCost_Entity
                    {
                        ...
                        , SoukoCD = mGrid.g_DArray[i].SoukoName    //未使用だが念のため
                    };
                    if (mibl.M_SKULastCost_Select(msce))
                    {
                        mGrid.g_DArray[i].CostUnitPrice = bbl.Z_SetStr(msce.LastCost);
                        mGrid.g_DArray[i].DefaultCostUnitPrice = bbl.Z_Set(msce.LastCost);
        ...
        private bool CheckDetail(int index)
        {
            ...
            switch (index)
            {
                case (int)EIndex.JuchuuDate:
                    ...
                    {
                        if (!InCaseOfLeaveEvent)
                        {
                            bbl.ShowMessage("E102");
                            mOldJyuchuDate = string.Empty;
                            detailControls[index].Focus();
                        }
                        return false;
                    }
                    ...
                    var formatDate = bbl.FormatDate(detailControls[index].Text);
                    if (!bbl.CheckDate(formatDate))
                    {
                        if (!InCaseOfLeaveEvent)
                        {
                    ...
                    detailControls[index].Text = formatDate;
                    ...
                    if (mOldJyuchuDate != detailControls[index].Text)
                    {
                        ...
                        for (int RW = 0; RW <= mGrid.g_MK_Max_Row - 1; RW++)
                        {
                            ...
                            {
                                var ret = CheckGrid((int)ClsGridJuchuu.ColNO.JanCD, RW, true, true, isOverwrite);
                                if (!ret && isOverwrite)
                                {
                                    SuppressLeaveEvent = true;
        ...
        /// <param name="overwrite">入力値をマスタ値で上書きする場合true</param>
        /// <returns></returns>
        private bool CheckDependsOnDate(int index, bool changeDate = false, bool overwrite = true)
        {
            bool isShowMessage = !changeDate || overwrite;
                    ...
                    if (ret)
                    {
                        ...
                        if (kbn.Equals(0))
                        {
                            ...
                            if (mOldCustomerCD != detailControls[index].Text || changeDate)
                            {
                                ...
                                if (string.IsNullOrWhiteSpace(ScDeliveryCD.TxtCode.Text) && overwrite)
                                {
        ...
        /// <param name="changeYmd">受注日変更時はtrue（trueの場合はメッセージを表示しない）</param>
        /// <param name="overwrite">受注日変更時に入力値をマスタ値で上書きする場合true</param>
        /// <param name="callerControlIndex">呼び出し元コントロールのIndex（既定値:-1 0:受注番号 1:複写元受注番号 2:見積番号 4:返品元受注番号)</param>
        /// <returns></returns>
        private bool CheckGrid(int col, int row, bool chkAll = false, bool changeYmd = false, bool overwrite = false, int callerControlIndex = -1)
        {
            bool isShowMessage = !changeYmd || overwrite;
            bool noHikiate = (callerControlIndex == (int)EIndex.JuchuuNO || callerControlIndex == (int)EIndex.MotoJuchuuNO);
            ...
            //JANCD
            if (col == (int)ClsGridJuchuu.ColNO.JanCD)
            {
                ...
                bool isChanged = overwrite || mGrid.g_DArray[row].JanCD != mGrid.PreInput[row].JanCD;
                bool isReCalc = isChanged;

                //販売単価 複写元受注番号が入力されている場合は、以下のメッセージを表示後、その回答によって扱いを変える
                if (!chkAll && !changeYmd && !string.IsNullOrWhiteSpace(keyControls[(int)EIndex.CopyJuchuuNO].Text))
                {
                    if (mGrid.g_DArray[row].CopyJuchuGyoNO > 0)
                    {
                        //「複写する受注の金額情報をコピーしますか？」
                        if (bbl.ShowMessage("Q318") != DialogResult.Yes)
                        {
                            //単価再計算する
                            mGrid.g_DArray[row].CopyJuchuGyoNO = 0; //コピー元の値は消えてしまうので次回以降メッセージを表示しない
                            isReCalc = true;
                        }
                    }
                }
                ...
                if ((!isChanged || callerControlIndex != -1))
                {
                    //セット品の場合は同じJanCDで複数存在する
                ...
                //値が変更された or 諸口でない
                if (isChanged || mGrid.g_DArray[row].VariousFLG == 0)
                {
                    //諸口の場合は入力項目
                ...
                if (isChanged)
                {
                    ...
                    if (vbl.M_Vendor_SelectTop1(mve))
                    {
                        mGrid.g_DArray[row].VendorName = mve.VendorName;
                    }
                    else
                    {
                        mGrid.g_DArray[row].VendorName = string.Empty;
                    }
                    mGrid.g_DArray[row].DefaultVendorCD = mGrid.g_DArray[row].VendorCD;
                    mGrid.g_DArray[row].DefaultVendorName = mGrid.g_DArray[row].VendorName;
                    ...
                    //Form.受注数＝0の場合は１とする
                    if (callerControlIndex == -1 && mGrid.g_DArray[row].JuchuuSuu.ToInt32(0) == 0)
                    {
                        mGrid.g_DArray[row].JuchuuSuu = "1";
                    }

                    //明細の出荷倉庫CDに、Header部の出荷倉庫CD、倉庫名をセットする。
                    CopyHeaderSoukoToGrid(row);
                ...
                if (isChanged || isReCalc)
                {
                    ...
                    if (bbl.Fnc_UnitPrice(fue))
                    {
                        ...
                        mGrid.g_DArray[row].DefaultCostUnitPrice = bbl.Z_Set(fue.GenkaTanka);
                    }
                    else
                    {
                        //販売単価		
                        ...
                        mGrid.g_DArray[row].DefaultCostUnitPrice = 0;
                    }
                    ...
                    //原価単価＝０の場合、発注単価を原価単価にセット
                    if (fue.GenkaTanka.ToDecimal(0) == 0)
                    {
                        mGrid.g_DArray[row].CostUnitPrice = mGrid.g_DArray[row].OrderUnitPrice;
                    }

                    //税抜販売額・税込販売額
                    SetJuchuuGaku(row, ymd, fue.ZeinukiTanka);
                    //原価額・粗利額
                    SetCostGaku(row);
                    //消費税
                    CalcZei(row, false, fue);
                }

                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.JanCD); //前回入力値を確定
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.VendorCD); //前回入力値を確定
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.JuchuuSuu); //前回入力値を確定
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.JuchuuUnitPrice); //前回入力値を確定
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.OrderUnitPrice); //前回入力値を確定
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.CostUnitPrice); //前回入力値を確定
                Grid_NotFocus(col, row);
            }

            //SKUName
            if (col == (int)ClsGridJuchuu.ColNO.SKUName)
            {
            ...
            //SoukoName
            if (col == (int)ClsGridJuchuu.ColNO.SoukoName)
            {
                if (mGrid.g_MK_State[col, row].Cell_Enabled)
                {
                    //必須入力(Entry required)、入力なければエラー(If there is no input, an error)Ｅ１０２
                ...
                if (!noHikiate) CheckHikiate(row, ymd);
            }

            //VendorCD
            if (col == (int)ClsGridJuchuu.ColNO.VendorCD)
            {
                bool isChanged = overwrite || mGrid.g_DArray[row].VendorCD != mGrid.PreInput[row].VendorCD;
                ...
                if (!string.IsNullOrWhiteSpace(mGrid.g_DArray[row].VendorCD))
                {
                    ...
                    if (vebl.M_Vendor_SelectTop1(me))
                    {
                    ...
                    if (isChanged)
                    {
                        string oTanka = GetTanka(row, ymd, true);
                        if (oTanka == "error")
                        {
                            return false;
                        }
                        mGrid.g_DArray[row].OrderUnitPrice = oTanka;
                    }

                    if (bbl.Z_Set(mGrid.g_DArray[row].OrderUnitPrice) != 0 && bbl.Z_Set(mGrid.g_DArray[row].DefaultCostUnitPrice) == 0)
                    {
            ...
            //TyokusoCheckBox
            if (col == (int)ClsGridJuchuu.ColNO.ChkTyokuso)
            {
                if (mGrid.g_DArray[row].ChkTyokuso)
                {
            ...
            //ShippingPlanDate
            if (col == (int)ClsGridJuchuu.ColNO.ShippingPlanDate)
            {
                //入力無くても良い(It is not necessary to input)
                if (!string.IsNullOrWhiteSpace(mGrid.g_DArray[row].ShippingPlanDate))
                {
                    bool isChanged = overwrite || mGrid.g_DArray[row].ShippingPlanDate != mGrid.PreInput[row].ShippingPlanDate;
                ...
                mGrid.CommitPreviousInput(row, ClsGridJuchuu.ColNO.ShippingPlanDate); //前回入力値を確定
            }

            //ArrivePlanDate
            if (col == (int)ClsGridJuchuu.ColNO.ArrivePlanDate)
            {
                //入力無くても良い(It is not necessary to input)
            ...
            //JuchuuSuu
            if (col == (int)ClsGridJuchuu.ColNO.JuchuuSuu)
            {
                bool isChanged = overwrite || mGrid.g_DArray[row].JuchuuSuu != mGrid.PreInput[row].JuchuuSuu;

                int inputSu;
                if (!int.TryParse(mGrid.g_DArray[row].JuchuuSuu, out inputSu))
                {
                    if (isShowMessage && !InCaseOfLeaveEvent)
                    {
                        //「該当の値を入力・選択してください。」
                        bbl.ShowMessage("E102");
                    }
                ...
                if (mGrid.g_DArray[row].NotPrintFLG)
                {
                    if (mGrid.g_DArray[row].JuchuuSuu != mGrid.g_DArray[row - 1].JuchuuSuu)
                    {
                        if (isShowMessage && !InCaseOfLeaveEvent)
                        {
                            //「「印刷なし」の場合、その上の行の数量と同じ値の必要があります」
                            bbl.ShowMessage("E217");
                            //「印刷なし」をOFFに修正した場合に数量を変更していないのに単価取得が走るため前回入力値はクリアしない
                        }
                        return false;
                    }
                }
            ...
            //JuchuuUnitPrice
            if (col == (int)ClsGridJuchuu.ColNO.JuchuuUnitPrice)
            {
                bool isChanged = overwrite || mGrid.g_DArray[row].JuchuuUnitPrice != mGrid.PreInput[row].JuchuuUnitPrice;

                if (isChanged)
                {
                    string tanka = mGrid.g_DArray[row].JuchuuUnitPrice;
                    if (mTennic.Equals(0))
                    {
                        tanka = bbl.GetZeinukiKingaku(bbl.Z_Set(mGrid.g_DArray[row].JuchuuUnitPrice), mGrid.g_DArray[row].TaxRateFLG, ymd).ToString();
                    }

                    SetJuchuuGaku(row, ymd, tanka);
                    SetCostGaku(row);
                    CalcZei(row, true);
                }
            ...
            //OrderUnitPrice
            if (col == (int)ClsGridJuchuu.ColNO.OrderUnitPrice) //発注単価
            {
                ...
                else if (bbl.Z_Set(mGrid.g_DArray[row].CostUnitPrice) == 0 || mGrid.g_DArray[row].DefaultCostUnitPrice == 0)
                {
                    mGrid.g_DArray[row].CostUnitPrice = mGrid.g_DArray[row].OrderUnitPrice;
                    SetCostGaku(row);
                }
            ...
            //CostUnitPrice
            if (col == (int)ClsGridJuchuu.ColNO.CostUnitPrice)
            {
                bool isChanged = overwrite || mGrid.g_DArray[row].CostUnitPrice != mGrid.PreInput[row].CostUnitPrice;
            ...
            if (col == (int)ClsGridJuchuu.ColNO.JanCD
             || col == (int)ClsGridJuchuu.ColNO.JuchuuSuu
             || col == (int)ClsGridJuchuu.ColNO.JuchuuUnitPrice
             || col == (int)ClsGridJuchuu.ColNO.CostUnitPrice
             || col == (int)ClsGridJuchuu.ColNO.OrderUnitPrice
            )
            {
                //各金額項目の再計算必要
                if (chkAll == false) CalcKin();
            }
                ...
                if (ret)
                {
                    if (fre.Result == "1")
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(fre.LastDay))
                        {
                            ...
                            if (fre.LastDay.CompareTo(bbl.GetDate()) <= 0)
                            {
                                mGrid.g_DArray[row].ArrivePlanDate = "";
                            }
                            else
                        ...
                        else
                        {
                            if (!string.IsNullOrWhiteSpace(detailControls[(int)EIndex.SalesDate].Text))
                            {
                                if (detailControls[(int)EIndex.SalesDate].Text.CompareTo(mGrid.g_DArray[row].ArrivePlanDate) < 0)
                                {
                    ...
                    else if (fre.Result == "2")
                    {
                        ...
                        if (string.IsNullOrEmpty(mGrid.g_DArray[row].VendorCD) && !string.IsNullOrEmpty(mGrid.g_DArray[row].DefaultVendorCD))
                        {
                            mGrid.g_DArray[row].VendorCD = mGrid.g_DArray[row].DefaultVendorCD;
                            mGrid.g_DArray[row].VendorName = mGrid.g_DArray[row].DefaultVendorName;
                        }
                        if (!string.IsNullOrWhiteSpace(fre.LastDay))
                        {
                            mGrid.g_DArray[row].ArrivePlanDate = fre.LastDay;
        ...
        private string GetTanka(int row, string ymd, bool showMessage = false)
        {
            //[M_JANOrderPrice]
        ...
        private void SetJuchuuGaku(int row, string ymd, string fZeinukiTanka)
        {
            decimal wSuu = mGrid.g_DArray[row].JuchuuSuu.ToDecimal(0);
        ...
        private void SetCostGaku(int row)
        {
            //原価額=原価単価×受注数
            mGrid.g_DArray[row].CostGaku = (mGrid.g_DArray[row].CostUnitPrice.ToDecimal(0) * mGrid.g_DArray[row].JuchuuSuu.ToDecimal(0)).ToString("#,##0");

            //粗利額←税抜販売額－原価額
            mGrid.g_DArray[row].ProfitGaku = (mGrid.g_DArray[row].JuchuuHontaiGaku.ToDecimal(0) - mGrid.g_DArray[row].CostGaku.ToDecimal(0)).ToString("#,##0");
        }

        private void CalcZei(int w_Row, bool changeTanka = false, Fnc_UnitPrice_Entity fue = null)
        {
            string ymd = detailControls[(int)EIndex.JuchuuDate].Text;
            ...
            for (int RW = 0; RW <= mGrid.g_MK_Max_Row - 1; RW++)
            {
                ...
                if (string.IsNullOrWhiteSpace(mGrid.g_DArray[RW].JanCD) == false)
                {
                    ...
                    dt.Rows.Add(mGrid.g_DArray[RW].juchuGyoNO > 0 ? mGrid.g_DArray[RW].juchuGyoNO : rowNo
                        ...
                        , mGrid.g_DArray[RW].SKUName == "" ? null : mGrid.g_DArray[RW].SKUName.GetByteString(80)
        ...
        protected override void ExecSec()
        {
            ...
            if (OperationMode != EOperationMode.DELETE)
            {
                ...
                for (int RW = 0; RW <= mGrid.g_MK_Max_Row - 1; RW++)
                {
                    if (string.IsNullOrWhiteSpace(mGrid.g_DArray[RW].JanCD) == false)
                    {
                        //JanCD未確定の状態でF12登録でいきなり更新されてしまうのを防ぐ
                        if (mGrid.g_DArray[RW].JanCD != mGrid.PreInput[RW].JanCD)
                        {
                            //「該当の値を入力・選択してください。」
                            bbl.ShowMessage("E102");
                            ERR_FOCUS_GRID_SUB((int)ClsGridJuchuu.ColNO.JanCD, RW);
        ...
        private void ChangeOperationMode(EOperationMode mode)
        {
            ...
            switch (mode)
            {
                case EOperationMode.INSERT:
                    ...
                    CboSoukoName.SelectedValue = SoukoCD;
        ...
        {
            //#Leave event
            //Raise 'Leave' event of the active control. --->
            ...
            switch (kbn)
            {
                case EsearchKbn.Product:
                    ...
                    using (Search_Product frmProduct = new Search_Product(detailControls[(int)EIndex.JuchuuDate].Text))
                    {
                        ...
                        frmProduct.VendorCD = mGrid.g_DArray[w_Row].VendorCD;
                        ...
                        if (!frmProduct.flgCancel)
                        {
                            ...
                            CheckGrid((int)ClsGridJuchuu.ColNO.JanCD, w_Row, false, false);
        ...
        private void DetailControl_Leave(object sender, EventArgs e)
        {
            try
            {
                if (SuppressLeaveEvent) return;
                if (InCaseOfLeaveEvent) return;

                if (sender == detailControls[(int)EIndex.JuchuuDate])
                {
                    InCaseOfLeaveEvent = true;
                    CheckDetail((int)EIndex.JuchuuDate);
                    InCaseOfLeaveEvent = false;
                }
            }
            catch (Exception ex)
            {
        ...
        private void GridControl_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                //#Leave event
                // 数量・単価はEnterキー押下と同様の処理
                if (w_Col == (int)ClsGridJuchuu.ColNO.JuchuuSuu
                 || w_Col == (int)ClsGridJuchuu.ColNO.JuchuuUnitPrice
                 || w_Col == (int)ClsGridJuchuu.ColNO.OrderUnitPrice)
                {
                    InCaseOfLeaveEvent = true;
                    GridControl_KeyDown(sender, new KeyEventArgs(Keys.Enter));
                    InCaseOfLeaveEvent = false;
                }
            }
            catch (Exception ex)
        ...
        private void GridControl_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                var w_ActCtl = (Control)sender;
                var w_Row = w_ActCtl.Tag.ToInt32(0) + Vsb_Mei_0.Value;
                    ...
                    ((e.KeyCode & (Keys.Alt | Keys.Control)) == Keys.None))
                {
                    ...
                    //行き先がなかったら移動しない
                    //#Leave event
                    if (!InCaseOfLeaveEvent) S_Grid_0_Event_Enter(CL, w_Row, w_ActCtl, w_ActCtl);
                }
        ...
        private void CboStoreCD_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var storeCD = CboStoreCD.SelectedValue.ToString();
                    var juchuuDate = detailControls[(int)EIndex.JuchuuDate].Text;

                    ScCustomer.Value2 = storeCD;
                    ScDeliveryCD.Value2 = storeCD;
                    CboSoukoName.StoreCD = storeCD;
                    CboSoukoName.Bind(juchuuDate);
                }
            }
            catch (Exception ex)
            {
        ...
        private void CopyHeaderSoukoToGrid(int rowIndex)
        {
            if (rowIndex >= mGrid.g_MK_Ctl_Row)
            {
                return;
            }

            var cboGridSouko = mGrid.g_MK_Ctrl[(int)ClsGridJuchuu.ColNO.SoukoName, rowIndex].CellCtl as CKM_Controls.CKM_ComboBox;
            if (cboGridSouko != null)
            {
                var dt = (CboSoukoName.DataSource as DataTable).Copy();
                cboGridSouko.BindCombo("SoukoCD", "SoukoName", dt);
                mGrid.g_DArray[rowIndex].SoukoName = CboSoukoName.SelectedValue.ToStringOrEmpty();
            }
        }
    }
}
