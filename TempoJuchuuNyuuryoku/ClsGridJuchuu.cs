...
//'明細部を構成するコントロールのうち TextBoxコントロール以外はカスタムコントロール(MeisaiControl)を使用してください。(Tabの検知のため)
    class ClsGridJuchuu :  GridBase.ClsGridBase
    {
        internal struct ST_DArray_Grid
        {
            ...
            internal decimal DefaultCostUnitPrice;      //原価単価 
            ...
            internal string DefaultVendorCD;
            internal string DefaultVendorName;
        }
        ...
        internal ST_DArray_Grid[] PreInput = null; //前回入力値（変更されたかどうか判定用）
        ...
        internal void CommitPreviousInput(int row, ColNO col = ColNO.None)
        {
            var current = this.g_DArray[row];

            if (col == ColNO.None || col == ColNO.JanCD) PreInput[row].JanCD = current.JanCD;
            if (col == ColNO.None || col == ColNO.JuchuuSuu) PreInput[row].JuchuuSuu = current.JuchuuSuu;
            if (col == ColNO.None || col == ColNO.JuchuuUnitPrice) PreInput[row].JuchuuUnitPrice = current.JuchuuUnitPrice;
            if (col == ColNO.None || col == ColNO.OrderUnitPrice) PreInput[row].JuchuuUnitPrice = current.OrderUnitPrice;
            if (col == ColNO.None || col == ColNO.CostUnitPrice) PreInput[row].CostUnitPrice = current.CostUnitPrice;
            if (col == ColNO.None || col == ColNO.VendorCD) PreInput[row].VendorCD = current.VendorCD;
            if (col == ColNO.None || col == ColNO.ShippingPlanDate) PreInput[row].ShippingPlanDate = current.ShippingPlanDate;
        }
        internal void ClearPreviousInput(int row, ColNO col = ColNO.None)
        {
            if (col == ColNO.None || col == ColNO.JanCD) PreInput[row].JanCD = string.Empty;
            if (col == ColNO.None || col == ColNO.JuchuuSuu) PreInput[row].JuchuuSuu = string.Empty;
            if (col == ColNO.None || col == ColNO.JuchuuUnitPrice) PreInput[row].JuchuuUnitPrice = string.Empty;
            if (col == ColNO.None || col == ColNO.OrderUnitPrice) PreInput[row].OrderUnitPrice = string.Empty;
            if (col == ColNO.None || col == ColNO.CostUnitPrice) PreInput[row].CostUnitPrice = string.Empty;
            if (col == ColNO.None || col == ColNO.VendorCD) PreInput[row].VendorCD = string.Empty;
            if (col == ColNO.None || col == ColNO.ShippingPlanDate) PreInput[row].ShippingPlanDate = string.Empty;
        }
    }
}
