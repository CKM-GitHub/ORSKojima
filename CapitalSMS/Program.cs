    ...
    static class Program
    {
        ...
        public static void CreateIfMissingAsync()
        {
            ...
            try
            {
                if (!File.Exists(@"C:\SMS\AppData\CKM.ini"))
                {
                    ...
                    text = text.Replace("=TennicMainMenuLogin", "=CapitalMainMenuLogin");
                    text = text.Replace("=HaspoMainMenuLogin", "=CapitalMainMenuLogin"); 
                    text = text.Replace("=HaspoStoreMenuLogin", "=CapitalMainMenuLogin");
                    text = text.Replace("=CapitalStoreMenuLogin", "=CapitalMainMenuLogin"); 
                    text = text.Replace("=CapitalMainMenuLogin", "=CapitalMainMenuLogin");
                    File.WriteAllText(@"C:\SMS\AppData\CKM.ini", text);
                }
                else
                {
                    string text = File.ReadAllText(@"C:\SMS\AppData\CKM.ini");
                    //if (text.Contains("=CapitalMainMenuLogin") || text.Contains("=CapitalStoreMenuLogin"))
                    //{
                    //    //ok
                    //}
                    //else
        ...
        static Form LoginFormName()
        {
            ...
            if (!System.Deployment.Application.ApplicationDeployment.IsNetworkDeployed)
            {
                ...
                localpath = localpath.Replace("CapitalSMS\\", "");
            }
            ...
            if (lbl.ReadConfig())
            {
                ...
                {
                    MessageBox.Show("The program cannot initialize with the specified server instaces  inside CKM.ini file. Please make sure capital's Store or MainMenu login type inside ini file!");
                    Environment.Exit(0);
                }
            }
