...
namespace TempoRegiRyousyuusyo
{
    ...
    public partial class TempoRegiRyousyuusyo : ShopBaseForm
    {
        ...
        /// <summary>
        /// DeveloperMode=1で販売登録から実行された場合にプレビューが固まるため表示しない
        /// </summary>
        private bool IsNotShowPreview = false;
        ...
        private void TempoRegiRyousyuusyo_Load(object sender, EventArgs e)
        {
           ...
           // string[] cmds = new string[] { "C:\\", "01", "0001", "ptk", "0001", "A0321090024", "0", "1", "2021/09/30", "1" };
            if (cmds.Length - 1 > (int)ECmdLine.PcID)
            {
                try
                {
                    //01 0001 ptk 0001 A0321090015 0 1 2021/09/30 1
                    // 別プログラムからの呼び出し時
        ...
        /// 印刷実行
        /// </summary>
        public void Print(string[] cmds = null)
        {
            if (cmds != null)
            {
                ...
                if (Base_DL.iniEntity.IsDM_D30Used)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(txtSalesNO.Text))
                        {
                            ...
                            if (chkReceipt.Checked)
                            {
                                ...
                                // 店舗取引履歴
                                DataTable receiptData = bl.D_ReceiptSelect(txtSalesNO.Text, StoreCD);
                        ...
                        if (!string.IsNullOrWhiteSpace(txtAmountOfMoney.Text))
                        {
                            DataTable printreceiptData = bl.D_RyousyuusyoPrintSelect(txtPrintDate.Text, txtAmountOfMoney.Text, InOperatorCD);
                            if (printreceiptData.Rows.Count > 0)
                            {
                                OutputRyouusyusyo(printreceiptData);
                            }
                        }
                    }
                //}
        ...
        private void OutputRyouusyusyo(DataTable data)
        {
            ...
            // 売上日付
            ryousyuusyoRow.UriageDateTime = ConvertDateTime(row["UriageDateTime"], true);
            ...
            // 消費税額
            var salesTax = Convert.ToString(row["SalesTax"]);
            //var salesTax = ConvertDecimal(row["SalesTax"]);
            //ryousyuusyoRow.SalesTax = (string.IsNullOrWhiteSpace(salesTax) || salesTax == "0" ? "" : @"\") + row["SalesTax"];
            ryousyuusyoRow.SalesTax = Convert.ToString(row["SalesTax"]);
         
            // 会社名
            //ryousyuusyoRow.CompanyName = Convert.ToString(row["CompanyName"]);
            ryousyuusyoRow.CompanyName = Convert.ToString(row["Print7"]);

            // 店舗名
            //ryousyuusyoRow.StoreName = Convert.ToString(row["StoreName"]);
            ryousyuusyoRow.StoreName = Convert.ToString(row["Print8"]);
        ...
        private void OutputReceipt(DataTable data)
        {
           ...
           // var imgLogo = bl.GetLogo(StoreCD);
            for (var index = 0; index < data.Rows.Count; index++)
            {
                ...
                //モード
                sales.CancelKBN = Convert.ToString(row["CancelKBN"]);                           // モード(2:返品 3:訂正)
        ...
        private void txtAmountOfMoney_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                btnClose.Focus();
