...
namespace NyuukaNyuuryoku
{
    ...
    internal partial class NyuukaNyuuryoku : FrmMainForm
    {
       ...
       private void DeleteExclusive()
        ...
        private void ERR_FOCUS_GRID_SUB2(int pCol, int pRow, Control w_Ctrl = null)
        {
            bool w_Ret;
            int w_CtlRow;
            ...
            if (w_Ctrl == null) w_Ctrl = detailControls[(int)EIndex.Nyukasu];
        ...
        private bool CheckGrid(int col, int row, ref bool focus, bool chkAll = false, bool changeYmd = false, bool changed = true)
        {
            ...
            switch (col)
            {
                case (int)ClsGridHikiate.ColNO.SURYO:
                    ...
                    {
                        //Ｅ１４３
                        if (!InCaseOfLeaveEvent) bbl.ShowMessage("E143", "1", "小さい");
                        return false;
                    }
                    //【引当】
                    //入荷数＞引当数の場合、エラー
                    ...
                    {
                        //Ｅ１４３
                        if (!InCaseOfLeaveEvent) bbl.ShowMessage("E143", "引当数", "大きい");
                        return false;
                    }
                    if (changed && bbl.Z_Set(mGrid.g_DArray[row].SURYO) > 0)
                    {
                    ...
                    if (bbl.Z_Set(mGrid.g_DArray[row].SURYO) != bbl.Z_Set(mGrid.g_DArray[row].ReserveSu))
                    {
                        ...
                        for (int RW = 0; RW <= mGrid2.g_MK_Max_Row - 1; RW++)
                        {
                            if (mGrid2.g_DArray[RW].StockNO == stockNO)
                            {
                                ...
                                {
                                    //Ｅ２５１
                                    if (!InCaseOfLeaveEvent) bbl.ShowMessage("E251");
                                    //Focusセット処理
        ...
        private bool CheckGrid2(int col, int row, bool chkAll = false, bool changeYmd = false, bool changed = true)
        {
            if (!chkAll && !changeYmd)
            {
            ...
            switch (col)
            {
                case (int)ClsGridHikiate.ColNO.SURYO:
                    ...
                    //    return false;
                    //}
                    if (changed && bbl.Z_Set(mGrid2.g_DArray[row].SURYO) > 0)
                    {
        ...
        private bool CalcKin(bool f12 = false)
        {
            ...
            {
                //Ｅ１４３
                if (!InCaseOfLeaveEvent) bbl.ShowMessage("E143", "入荷総数", "大きい");
                return false;
            }
            if(f12)
            {
            ...
            try
            {
                    ...
                    ((e.KeyCode & (Keys.Alt | Keys.Control)) == Keys.None))
                {
                    ...
                    if (ret)
                    {
                        ...
                        {
                            //明細の先頭項目へ
                            mGrid.F_MoveFocus((int)ClsGridBase.Gen_MK_FocusMove.MvSet, (int)ClsGridBase.Gen_MK_FocusMove.MvNxt, ActiveControl, -1, -1, ActiveControl, Vsb_Mei_0, Vsb_Mei_0.Value, (int)ClsGridHikiate.ColNO.Check);

                            if (mGrid.F_Search_Ctrl_MK(this.ActiveControl, out int w_Col, out int w_CtlRow) == false)
                            {
                                //明細の先頭項目へ
                                mGrid2.F_MoveFocus((int)ClsGridBase.Gen_MK_FocusMove.MvSet, (int)ClsGridBase.Gen_MK_FocusMove.MvNxt, ActiveControl, -1, -1, ActiveControl, Vsb_Mei_1, Vsb_Mei_1.Value, (int)ClsGridZaiko.ColNO.Check);
                            }
                        }
                        else if (index == (int)EIndex.VendorDeliveryNo)
                        {
                            if (string.IsNullOrEmpty(detailControls[index + 1].Text))
                            {
                                detailControls[index + 1].Focus(); //SoukoName
                            }
                            else
                            {
                                detailControls[index + 2].Focus(); //JanCD
                            }
                        }
        ...
        private void GridControl_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                //#Leave event
                // 数量はEnterキー押下と同様の処理
                if (w_Col == (int)ClsGridHikiate.ColNO.SURYO)
                {
        ...
        private void GridControl_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                    ...
                    ((e.KeyCode & (Keys.Alt | Keys.Control)) == Keys.None))
                {
                    ...
                    if (CL == (int)ClsGridHikiate.ColNO.ChkFinish)
                    {
                        ...
                        {
                            lastCell = string.IsNullOrEmpty(mGrid.g_DArray[w_Row + 1].JYUNO);
                        }
                    }
                    ...
                    {
                        changed = mGrid.g_DArray[w_Row].SURYO.ToInt32(0) != w_ActCtl.Text.ToInt32(0);
                    }
                    ...
                    //チェック処理
                    if (CheckGrid(CL, w_Row, ref focus, false, false, changed) == false)
                    {
                        //配列の内容を画面へセット
                        mGrid.S_DispFromArray(Vsb_Mei_0.Value, ref Vsb_Mei_0);
                        if (!InCaseOfLeaveEvent && focus)
                            //Focusセット処理
                            w_ActCtl.Focus();
                        return;
                    }
                    ...
                    if (lastCell)
                    {
                        ERR_FOCUS_GRID_SUB2((int)ClsGridZaiko.ColNO.Check, 0, w_ActCtl);
                        return;
                    }
        ...
        private void GridControl_Leave2(object sender, EventArgs e)
        {
            try
            {
                ...
                //#Leave event
                // 数量はEnterキー押下と同様の処理
                if (w_Col == (int)ClsGridZaiko.ColNO.SURYO)
                {
                    InCaseOfLeaveEvent = true;
                    GridControl_KeyDown2(sender, new KeyEventArgs(Keys.Enter));
                    InCaseOfLeaveEvent = false;
                }
            }
            catch (Exception ex)
        ...
        private void GridControl_KeyDown2(object sender, KeyEventArgs e)
        {
            try
            {
                    ...
                    ((e.KeyCode & (Keys.Alt | Keys.Control)) == Keys.None))
                {
                    ...
                    if (CL == (int)ClsGridZaiko.ColNO.SURYO)
                    {
                        changed = mGrid2.g_DArray[w_Row].SURYO.ToInt32(0) != w_ActCtl.Text.ToInt32(0);
                    }
                    ...
                    //チェック処理
                    if (CheckGrid2(CL, w_Row, false, false, changed) == false)
                    {
                        //Focusセット処理
                        if (!InCaseOfLeaveEvent) w_ActCtl.Focus();
                        return;
                    }
                    ...
                    //this.ProcessTabKey(!e.Shift);
                    //行き先がなかったら移動しない
                    if (!InCaseOfLeaveEvent) S_Grid_1_Event_Enter(CL, w_Row, w_ActCtl, w_ActCtl);
                }
                else if (e.KeyCode == Keys.Tab)
                {
                    if (mGrid2.F_Search_Ctrl_MK(w_ActCtl, out int CL, out int w_CtlRow) == false)
