    ...
    public partial class TempoRegiJournal : ShopBaseForm
    {
        ...
        private StoreDataSet CreateStoreDataSet(DataTable data, bool isPrint)
        {
            ...
            for (var index = 0; index < data.Rows.Count; index++)
            {
                ...
                var dispSalesNO = Convert.ToString(row["DispSalesNO"]);                     // 表示用売上番号
                ...
                var issueDateTime = Convert.ToString(row["BreakIssueDate"]) + " " + Convert.ToString(row["RecoredKBN"]); // 発行日時(yyyy/mm/dd HH:mm:dd形式)
                var dispIssueDateTime = ConvertDateTime(row["IssueDate"], false);               // 発行日時
                var cancelKBN = Convert.ToString(row["CancelKBN"]);                         // 取消区分
                ...
                if (index == 0)
                {
                    ...
                    store.CompanyName = Convert.ToString(row["CompanyName"]);                   // 会社名
                ...
                if (!string.IsNullOrWhiteSpace(salesNO))
                {
                    ...
                    salesNo.DispSalesNO = dispSalesNO;                                                                  // 表示用売上番号
                    ...
                    salesNo.DispIssueDateTime = dispIssueDateTime;  　  　　　　　　　　　　　　　　　　　　　　　　　　// 表示用発行日時
                    salesNo.CancelKBN = cancelKBN;                                                                      // 取消区分
                ...
                if (OldDepositNo != row["DepositNO"].ToString() && !string.IsNullOrWhiteSpace(salesNO))
                {
                    ...
                    sale.DispSalesNO = dispSalesNO;                                                                 // 表示用売上番号
                    ...
                    sale.DispIssueDateTime = dispIssueDateTime;                                                     // 発行日時
                    sale.DispDateTime = "";
                    sale.CancelKBN = cancelKBN;                                                     // 取消区分
                ...
                if (storeDataSet.CashBalanceTable.Where(v => v.RegistDate == registDate).FirstOrDefault() == null)
                {
                    ...
                    decimal kyakuTanka = 0;                                                                 // 総売 客単価
                    if (cashBalance.CustomerCDCount.ToInt32(0) != 0)
                    {
                        kyakuTanka = Math.Round(cashBalance.TotalGakuSum.ToDecimal(0) / cashBalance.CustomerCDCount.ToDecimal(0));
                    }
                    cashBalance.KyakuTanka = ConvertDecimal(kyakuTanka);
                    #endregion // 精算処理 総売
            ...
            // SalesNoTable行一括追加
            AddSalesNoTableRows(storeDataSet.SalesNoTable, salesNos.OrderBy(v => v.IssueDateTime.ToDateTime()).ToList());

            // SalesTable行一括追加
            //AddSalesTableRows(storeDataSet.SalesTable, sales.OrderBy(v => v.IssueDateTime.ToDateTime()).ThenBy(v => v.SalesNO).ThenBy(v => v.Row).ToList());
            AddSalesTableRows(storeDataSet.SalesTable, sales.OrderBy(v => v.DispIssueDateTime.ToDateTime()).ThenBy(v => v.SalesNO).ThenBy(v => v.Row).ToList());
        ...
        private void SetMiscDepositTable(string salesNO, StoreDataSet.SalesTableDataTable sales, StoreDataSet.SalesNoTableDataTable salesNos, string calendarDate, object date, object name, object amount, string remark, string storeReceiptPrint, string staffReceiptPrint)
        {
            ...
            item.DispIssueDateTime = string.IsNullOrWhiteSpace(dateTime) ? DATETIME_MIN : dateTime;
                         ...
                         j.Remark == item.Remark).SingleOrDefault() == null)
            {
                ...
                salesNo.DispIssueDateTime = item.DispIssueDateTime;   // 表示用発行日時
        ...
        private void SetDepositTable(string salesNO, StoreDataSet.SalesTableDataTable sales, StoreDataSet.SalesNoTableDataTable salesNos, string calendarDate, object date, string customerCD, string customerName, object name, object amount, string remark, string storeReceiptPrint, string staffReceiptPrint)
        {
                         ...
                         j.Remark == item.Remark).SingleOrDefault() == null)
            {
                ...
                salesNo.IssueDate = item.IssueDate;                   // 発行日
                salesNo.IssueDateTime = item.IssueDateTime;           // 発行日時
                salesNo.DispIssueDateTime = item.DispIssueDateTime; 　// 発行日時
        ...
        private void AddStoreTableRow(StoreDataSet.StoreTableDataTable stores, StoreDataSet.StoreTableRow store)
        {
            ...
            row.CompanyName = store.CompanyName;                    // 会社名
        ...
        private StoreDataSet.SalesNoTableRow CreateSalesNoTableRow(StoreDataSet.SalesNoTableDataTable salesNos)
        {
            ...
            salesNo.DispSalesNO = "";                   // 表示用売上番号
            salesNo.IssueDate = "";                     // 発行日
            salesNo.IssueDateTime = "";                 // 発行日時
            salesNo.DispIssueDateTime = "";             // 表示用発行日時
            salesNo.CancelKBN = "";                     // 取消区分
        ...
        private void AddSalesNoTableRow(StoreDataSet.SalesNoTableDataTable salesNos, StoreDataSet.SalesNoTableRow salesNo)
        {
            ...
            row.DispSalesNO = salesNo.DispSalesNO;                      // 表示用売上番号
            row.IssueDate = salesNo.IssueDate;                          // 発行日
            row.IssueDateTime = salesNo.IssueDateTime;                  // 発行日時
            row.DispIssueDateTime = salesNo.DispIssueDateTime;          // 表示用発行日時
            row.CancelKBN = salesNo.CancelKBN;                          // 取消区分
        ...
        private StoreDataSet.SalesTableRow CreateSalesTableRow(StoreDataSet.SalesTableDataTable sales, JournalDataKind dataKind)
        {
            ...
            sale.SalesNO = "";                      // 売上番号
            sale.DispSalesNO = "";                  // 表示用売上番号
            sale.IssueDate = "";                    // 発行日
            sale.IssueDateTime = "";                // 発行日時
            sale.DispIssueDateTime = "";            // 表示用発行日時
        ...
        private void AddSalesTableRow(StoreDataSet.SalesTableDataTable sales, StoreDataSet.SalesTableRow sale)
        {
            ...
            row.DispSalesNO = sale.DispSalesNO;                     // 表示用売上番号
            row.IssueDate = sale.IssueDate;                         // 発行日
            row.IssueDateTime = sale.IssueDateTime;                 // 発行日時
            row.DispIssueDateTime = sale.DispIssueDateTime;         // 表示用発行日時
            row.DispDateTime = sale.DispDateTime;                   // 表示用発行日時
            row.CancelKBN = sale.CancelKBN;                         // 取消区分
