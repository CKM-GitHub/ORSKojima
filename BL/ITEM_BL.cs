    ...
    public class ITEM_BL : Base_BL
    {
        ...
        public bool M_ITEM_Select(M_ITEM_Entity me)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                me.TagName1 = dt.Rows[0]["TagName01"].ToString();
                me.TagName2 = dt.Rows[0]["TagName02"].ToString();
                me.TagName3 = dt.Rows[0]["TagName03"].ToString();
                me.TagName4 = dt.Rows[0]["TagName04"].ToString();
                me.TagName5 = dt.Rows[0]["TagName05"].ToString();
                me.TagName6 = dt.Rows[0]["TagName06"].ToString();
                me.TagName7 = dt.Rows[0]["TagName07"].ToString();
                me.TagName8 = dt.Rows[0]["TagName08"].ToString();
                me.TagName9 = dt.Rows[0]["TagName09"].ToString();
                me.TagName10 = dt.Rows[0]["TagName10"].ToString();
        ...
        /// <summary>
        /// ItemCDでM_Itemを検索して1件でも存在したらtrueを返却する
        /// </summary>
        /// <param name="itemCD"></param>
        /// <returns></returns>
        public bool M_ITEM_CheckExistsByItemCD(string itemCD)
        {
            var dt = mdl.M_ITEM_CheckExistsByItemCD(new M_ITEM_Entity() { ITemCD = itemCD });
        ...
        public DataTable GetGridEntity(DataTable dtSKU)
        {
            ...
            foreach (DataRow row in dtSKU.Rows)
            {
                ...
                dt.Rows.Add(
                 ...
                 , row["MakerVendorCD"].ToString() == "" ? null : row["MainVendorCD"]
                 ...
                 , Z_Set(row["SetSU"])
                 , Z_Set(row["DeleteFlg"])
                 , Z_Set(row["AdminNO"]) == 0 ? 0: 1     //UpdateFlg → 2021/09/16 ストアド内で判定するため未使用
                 );
