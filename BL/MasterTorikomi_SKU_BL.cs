...
namespace BL
{
   public class MasterTorikomi_SKU_BL:Base_BL
   {
        ...
        public bool M_SKU_CheckSKUExists(M_SKU_Entity info)
        {
            var dt = mtdl.M_SKU_CheckSKUExists(info);
            return dt == null || dt.Rows.Count == 0 ? false : true;
        }

        public bool M_SKU_CheckDuplicateSizeAndColor(M_SKU_Entity info)
        {
            var dt = mtdl.M_SKU_CheckDuplicateSizeAndColor(info);
            return dt == null || dt.Rows.Count == 0 ? false : true;
        }

        public bool M_SKU_CheckItemExists(M_SKU_Entity info)
        {
            var dt = mtdl.M_SKU_CheckItemExists(info);
            return dt == null || dt.Rows.Count == 0 ? false : true;
        }
