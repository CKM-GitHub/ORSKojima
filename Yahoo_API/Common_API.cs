    ...
    public class Common_API
    {
        ...
        public void Search_GetOrderDetail(D_APIControl_Entity DApiControl_entity)
        {
            ...
            for (int i = 0;i<3;i++)
            {
                ...
                api.RefreshToken = "A3a5f2EBAESSrEt1x-Bhp6d-j390qggaZNClW3wN6zzF54cWm01DZ0lmPURLs3GZntvQ_905DafBNU2-uvVcGrAEuqErczmADM5vpZOXi9Ivke4nDcPcKkcECr3hR-IXpkTKD5fa30byOfE0sFCzu0adJw9oV1bmboz0BSJWYJv7XCD6~1";// dtSecretKey.Rows[0]["RefreshToken"].ToString();
                //Found:
                if (YahooAPIAuth(api).Equals("ng"))
                {
                    string url = "https://auth.login.yahoo.co.jp/yconnect/v2/authorization?response_type=code&client_id=" + api.ApplicationID + "&redirect_uri=http://shopping.geocities.jp/racket/index.html&state=xyz";
                    string code = "MzxTiF1A";  // RedirectPath(url);
        ...
        public void SearOrder(M_API_Entity api) 
        {
            if (api.TESTMode.Equals("1"))
                countRequest = (HttpWebRequest)WebRequest.Create(string.Format("https://circus.shopping.yahooapis.jp/ShoppingWebService/V1/orderCount?sellerId={0}", api.SellerId));
            else
                countRequest = (HttpWebRequest)WebRequest.Create(string.Format("https://test.circus.shopping.yahooapis.jp/ShoppingWebService/V1/orderCount?sellerid={0}", api.SellerId));
