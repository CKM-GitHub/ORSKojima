    ...
    public partial class TempoRegiMasterNyuuryoku : ShopBaseForm
    {
        ...
        private bool ErrorCheck(int type)
        {
            ...
            if(type==1)
            {
                ...
                string str1 = Encoding.GetEncoding(932).GetByteCount(txtButtomNameUp.Text).ToString();  //TaskNo3322 HET
                if (Convert.ToInt32(str1) > txtButtomNameUp.MaxLength)
                {
                ...
                string str2 = Encoding.GetEncoding(932).GetByteCount(txtBtnNameDown.Text).ToString();  //TaskNo3322 HET
                if (Convert.ToInt32(str2) > txtBtnNameDown.MaxLength)
                {
                    MessageBox.Show("入力された文字が長すぎます", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void SearchDate_ByOption()
        {
            ...
            if (RdoJanCD.Checked)
            {
                ...
                if(!string.IsNullOrWhiteSpace(tpoShouhin.JANCD))
                {
                    ...
                    txtBtnNameDown.Text = tpoShouhin.SKUName;
                }
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(tgkkk.CustomerCD))
                {
                    ...
                    txtBtnNameDown.Text= tgkkk.CustomerName;
                ...
                if (RdoJanCD.Checked)
                {
                    ...
                    if (dtSKU.Rows.Count > 0)
                    {
                        if (dtSKU.Rows.Count == 1)
                        {
                            ...
                            txtBtnNameDown.Text = dtSKU.Rows[0]["SKUName"].ToString();
                        }
                        else
                        {
                    ...
                    if (dtCusto.Rows.Count > 0)
                    {
                        ...
                        if(dr.Count()==0)
                        {
                            if (dtCusto.Rows.Count == 1)
                            {
                                ...
                                txtBtnNameDown.Text= dtCusto.Rows[0]["CustomerName"].ToString();
                            }
                            lblSearchName.Visible = true;
                            lblAdminNO.Text = string.Empty;
                            txtBtnNameDown.Focus();
