...
namespace MasterTouroku_Tokuisaki
{
    ...
    internal partial class MasterTouroku_Tokuisaki : FrmMainForm
    {
        ...
        private void InitialControlArray()
        {
            ...
            detailControls = new Control[] { ChkVariousFLG,ChkBillingFLG,ChkCollectFLG,pnlCustomerKBN,CboStoreCD,ScStaff.TxtCode,checkDeleteFlg
                            ...
                            ,cmbPaymentMethodCD,ScKouzaCD.TxtCode,cmbPaymentUnit,ScTankaCD.TxtCode,ChkAttentionFLG,ChkConfirmFLG,txtConfirmComment
            ...
            ckM_RadioButton1.CheckedChanged += new System.EventHandler(RadioCustomerKBN_CheckedChanged);
            ...
            if (index == (int)EIndex.ChangeDate)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        lblStoreName.Text = mce.StoreName;
            ...
            switch (index)
            {
               // case (int)EIndex.CustomerCD: break;
                ...
                case (int)EIndex.CollectCD:
                    ...
                    //DenominationKBN_BL dbl = new DenominationKBN_BL();
                    //string SystemKBN;
                    //if (cmbPaymentMethodCD.SelectedValue==null)
                    //{
                    //    SystemKBN = "";
                    //}
                    //else if (cmbPaymentMethodCD.SelectedValue.ToString() == "-1")
                    //{
                    //    SystemKBN = "";
                    //}
                    //else
                    //{
                    //    SystemKBN = cmbPaymentMethodCD.SelectedValue.ToString();
                    //}
                    //DataTable dtkbn = dbl.M_DenominationKBN_Selectkbn(SystemKBN);

                    //if (dtkbn.Rows.Count>0)
                    //{
                    //    if (dtkbn.Rows[0]["SystemKBN"].ToString() == "5")
                    //    {
                    //        //入力必須(Entry required)
                    //        if (!RequireCheck(new Control[] { detailControls[index] }))
                    //        {
                ...
                case (int)EIndex.KouzaCD:
                    ...
                    if (detailControls[(int)EIndex.CollectCD].Text.Equals(keyControls[(int)EIndex.CustomerCD].Text))
                    {
                        //入力必須(Entry required)
                        //if (!RequireCheck(new Control[] { detailControls[index] }))
                        //{
        ...
        private M_Customer_Entity GetEntity(short kbn)
        {
            ...
            //pnlCustomerKBN.1:店舗会員、2:店舗現金会員、3:団体法人、4:セール
            ...
            else if (ckM_RadioButton1.Checked)
            {
                mce.CustomerKBN = "5";
                mce.StoreTankaKBN = "5";
            }
        ...
        private void SetEnabled(int kbn)
        {
            if (base.OperationMode == EOperationMode.INSERT || base.OperationMode == EOperationMode.UPDATE)
            {
                if (radioButton3.Checked)
                {
                    ...
                    //detailControls[(int)EIndex.LastName].Text = "";//2021/12/10 By SES
                    detailControls[(int)EIndex.FirstName].Enabled = false;
                    //detailControls[(int)EIndex.FirstName].Text = "";//2021/12/10 By SES
                }
                else
                {
        ...
        private void RadioCustomerKBN_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (base.OperationMode == EOperationMode.INSERT && string.IsNullOrEmpty(ScCopyCustomer.TxtCode.Text))
                {
                    if (radioButton4.Checked)
                    {
                        //得意先会員区分＝Webの場合 以下の初期値をセットする		
                        mc.StoreKBN = "4";//taskno 3241
                        ...
                        ScTankaCD.LabelText = string.Empty;
        ...
        {
            CboStoreCD.Text = string.Empty;
            ScStaff.Clear();
            ScKouzaCD.Clear();
            cmbPaymentMethodCD.Text = string.Empty;
            cmbPaymentUnit.Text = string.Empty;
            cmbTaxTiming.Text = string.Empty;
            cmbTaxFractionKBN.Text = string.Empty;
            cmbAmountFractionKBN.Text = string.Empty;
            cmbCreditLevel.Text = string.Empty;
           // cmbStoreTankaKBN.Text = string.Empty;
            txtCreditCheckKBN.Clear();
            lblPoint.Text = string.Empty;
            ChkDMFlg.Checked = false;
            ckM_RadioButton4.Checked = true;
            ckM_RadioButton5.Checked = false;
            ckM_RadioButton6.Checked = false;
            radioButton9.Checked = true;
            ckM_RadioButton2.Checked = true;
        }
        ...
        private void RadioButton_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                        ...
                        ((e.KeyCode & (Keys.Alt | Keys.Control)) == Keys.None))
                {
                    ...
                    {
                        if (detailControls[(int)EIndex.CboStoreCD].CanFocus)
                            detailControls[(int)EIndex.CboStoreCD].Focus();
                        else
                            //あたかもTabキーが押されたかのようにする
                            //Shiftが押されている時は前のコントロールのフォーカスを移動
                            this.ProcessTabKey(!e.Shift);
